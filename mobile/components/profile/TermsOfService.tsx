...
export function TermsOfService({ onClose }: TermsOfServiceProps) {
  ...
  const termsOfServiceText = t('profile.termsOfService.content', { returnObjects: true }) as string[];
  ...
  return (
    ...
    >
      <View className="flex-row justify-between items-center p-4 border-b border-border">
        <Text className="text-2xl font-bold text-foreground">{t('profile.termsOfService.title')}</Text>
        {onClose && (
          <TouchableOpacity 
            onPress={onClose}
      ...
      <ScrollView className="flex-1 p-4">
        {termsOfServiceText.map((paragraph, index) => (
